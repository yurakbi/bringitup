...
window.addEventListener('DOMContentLoaded', function () {
  ...
  slider.render();
  var modulPageSlider = new _modules_slider_slider_main__WEBPACK_IMPORTED_MODULE_0__["default"]({
    container: '.moduleapp',
    btns: '.next'
  });
    ...
    value: function init() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          statusMessage.textContent = _this.messages.loading;
          var formData = new FormData(item);

          _this.postData(_this.path, formData).then(function (res) {
            console.log(res);
            statusMessage.textContent = _this.messages.success;
          }).catch(function () {
            statusMessage.textContent = _this.messages.failure;
          }).finally(function () {
            _this.clearInputs();
...
function () {
  ...
  _createClass(VideoPlayer, [{
    ...
    key: "cratePlayer",
    value: function cratePlayer(url) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function bindTriggers() {
      ...
      this.nextModule.forEach(function (item) {
        item.addEventListener('click', function (e) {
          // всплиття собитий (переключення вперед черещ один)
          e.stopPropagation();
          e.preventDefault();
